...
export default function EventCalendar() {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (data.duplicatesRemoved > 0) {
          setSuccessMsg(
            `Cleaned up ${data.duplicatesRemoved} duplicate event(s)`
          );
        }
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        eventDate: dateInputToUTC(formData.eventDate), // Convert to UTC midnight
      };
      ...
      const response = await fetch(BACKEND_URL, {
        ...
        body: JSON.stringify(payload), // ✅ Send converted payload
      });
      ...
      if (data.ok) {
        ...
        if (data.duplicate) {
          setErrorMsg(
            "This event already exists! A duplicate event was not created."
          );
        } else {
